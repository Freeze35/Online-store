let devices = require("./devicesApi.json")
...
const proxy = {
    ...
    //Get All devices,type,brand,typeBrand
    'GET /api/device': (req, res) => {
        ...
        if (!brandId && !typeId) {
            ...
            for (let i = 0; limitDevs(i, devices); i++) {
                let idOffset = offset + i
                newDevices["rows"] = [...newDevices.rows, devices.rows[idOffset]]
                newDevices["count"] = devices.count
            }
        }
        ...
        if (brandId && !typeId) {
            const devRows = () => devices.rows.filter(
    ...
    'GET /api/device/:id': (req, res) => {
        ...
        dev = {...dev, "info": [...devicesInfo.filter(info => ~~info.deviceId === ~~id)]}
        return res.json(dev)
    },
